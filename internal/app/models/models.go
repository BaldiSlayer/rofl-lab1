...
func LoadQABase() ([]QAPair, error) {
  ...
  {"question": "Что такое ДКА?", "answer": "детерминированный конечный автомат"},
  {
      ...
      "answer": "Языки, допускаемые машинами Тьюринга, называются рекурсивно-перечислимыми (РП), а РП-языки, допускаемые МТ, которые всегда останавливаются, — рекурсивным. Язык L называется рекурсивным, если L = L(M) для некоторой машины Тьюринга M, удовлетворяющей следующим условиям. 1. Если w принадлежит L, то M попадает в допускающее состояние (и, следовательно, останавливается). 2. Если w не принадлежит L, то M в конце концов останавливается, хотя и не попадает в допускающее состояние. МТ этого типа соответствует интуитивному понятию “алгоритма” — правильно определенной последовательности шагов, которая всегда заканчивается и приводит к некоторому ответу. Если мы рассматриваем язык L как “проблему”, то проблема L называется разрешимой, если она является рекурсивным языком. В противном случае проблема называется неразрешимой."
  },
  {
      "question": "Рекурсивно-перечислимые языки. Примеры языков, которые являются рекурсивно-перечислимыми, но не рекурсивными.",
      "answer": "Язык L является рекурсивно-перечислимым (РП-языком), если L = L(M) для некоторой машины Тьюринга M. Набор остановки машины Тьюринга является рекурсивно перечислимого, но не рекурсивно. Действительно, можно запустить машину Тьюринга и принять, если машина останавливается, следовательно, она рекурсивно перечислима. С другой стороны, проблема неразрешима."
  },
  {
      "question": "Что такое язык диагонализации L_d",
      "answer": "Язык диагонализации Ld — это множество всех цепочек wi, не принадлежащих L(Mi). Понятие Mi, “i-й машины Тьюринга”. Это МТ M, кодом которой является i-я двоичная цепочка wi. В этот язык входит каждая цепочка в алфавите {0, 1}, которая, будучи проинтерпретированной как код МТ, не принадлежит языку этой МТ. Язык Ld является хорошим примером не РП-языка, т.е. его не допускает ни одна машина Тьюринга."
  },
  {
  ...
  },
  {
      ...
      "answer": "Язык Lu является РП. Допустим, что Lu рекурсивен. Тогда по теореме L' u (дополнение Lu) — также рекурсивный язык. Но если существует МТ M, допускающая L' u, то, используя описанный ниже метод, можно построить МТ, допускающую Ld. Поскольку нам известно, что Ld не является РП, приходим к противоречию с предположением, что язык Lu является рекурсивным. Предположим, что L(M) = L' u. можно преобразовать МТ M в МТ M′, которая допускает Ld с помощью следующих действий1. M′ преобразует входную цепочку w в w111w. В качестве упражнения читатель может написать программу для выполнения этого шага на одной ленте. Однако легче это сделать, используя для копии w вторую ленту, и затем преобразовать двухленточную МТ в одноленточную. 2. M′ имитирует M на новом входе. Если w есть wi в нашем перечислении, то M′ определяет, допускает ли Mi вход wi. Поскольку M допускает L' u, то она допускает тогда и только тогда, когда Mi не допускает wi, т.е. когда wi принадлежит Ld. Таким образом, M′ допускает w тогда и только тогда, когда w принадлежит Ld. Поскольку по теореме машины M′ не существует, приходим к выводу, что язык Lu не является рекурсивным."
  },
  {
      "question": "Является ли язык не содержащий пустого слова Lne РП/рекурсивным?",
      "answer": "Lne – непустой язык состоит из кодов всех машин Тьюринга, которые допускают хотя бы одну цепочку.   Он рекурсивно-перечислим, но не рекурсивным. Чтобы доказать РП достаточно предъявить МТ, допускающую Lne. Проще всего это сделать, описав недетерминированную МТ M. абота M заключается в следующем. 1. На вход M подается код МТ Mi. 2. Используя недетерминизм, M угадывает вход w, который, возможно, допускается Mi. 3. M проверяет, допускает ли Mi свой вход w. В этой части M может моделировать работу универсальной МТ U, допускающей Lu. 4. Если Mi допускает w, то и M допускает свой вход, т.е. Mi. Понятие Mi, “i-й машины Тьюринга”. Это МТ M, кодом которой является i-я двоичная цепочка wi. В этот язык входит каждая цепочка в алфавите {0, 1}, которая, будучи проинтерпретированной как код МТ, не принадлежит языку этой МТ.Таким образом, если Mi допускает хотя бы одну цепочку, то M угадает ее (среди прочих, конечно) и допустит Mi. Если же L(Mi) = ∅, то ни одна из угаданных w не допускается Mi, и M не допустит Mi. Таким образом, L(M) = Lne."
  },
  {
      "question": "Является ли пустой язык состоит из кодов всех МТ, языки которых пусты Le РП/рекурсивным?",
      "answer": " Le – пустой язык состоит из кодов всех МТ, языки которых пусты. Он не рекурсивно-перечислим."
  },
  {
      "question": "Теорема Райса",
      "answer": "Всякое нетривиальное свойство языков, допускаемых МТ, является неразрешимым. Например, множество кодов машин Тьюринга, допускающих пустой язык, согласно теореме Райса является неразрешимым. В действительности этот язык не является РП, хотя его дополнение — множество кодов МТ, допускающих хотя бы одну цепочку, — является РП, но не рекурсивным."
  },
  {
  ...
  },
  {
      ...
      "answer": "Заданы два списка, содержащие одинаковое количество цепочек. Спрашивается, можно ли, выбирая последовательности соответствующих цепочек из этих двух списков, построить путем их конкатенации одну и ту же цепочку. ПСП является важным примером неразрешимой проблемы. Сводимость ПСП к ряду других проблем обеспечивает доказательство их неразрешимости"
  },
  {
      "question": "Разрешим ли вопрос о неоднозначности КС-грамматики",
      "answer": "Вопрос о неоднозначности КС-грамматики неразрешим. Неразрешимые проблемы, связанные с контекстно-свободными языками. Посредством сведения ПСП можно доказать неразрешимость многих вопросов о КСязыках или их грамматиках. Так, например, неразрешимы вопросы о неоднозначности КС-грамматики, о включении одного КС-языка в другой или о пустоте пересечения двух КС-языков."
  },
  {
      "question": "Пусть G1 и G2 — КС-грамматики, а R — регулярное выражение. Тогда какие проблемы неразрешимы?",
      "answer": "Пусть G1 и G2 — КС-грамматики, а R — регулярное выражение. Тогда неразрешимы следующие проблемы: а) 'L(G1) I L(G2) = ∅ ?' б) 'L(G1) = L(G2) ?' в) 'L(G1) = L(R) ?'' г) верно ли, что L(G1) = T* для некоторого алфавита T ? д) 'L(G1) ⊆ L(G2) ?' е) 'L(R) ⊆ L(G1) ?'. Доказательство. Каждое из доказательств представляет собой сведение ПСП. Покажем, как преобразовать экземпляр (A, B) ПСП в вопрос о КС-грамматиках и/или регулярных выражениях, который имеет ответ “да” тогда и только тогда, когда данный экземпляр ПСП имеет решение. В некоторых случаях ПСП сводится к самому вопросу, сформулированному в теореме, в других же случаях — к его дополнению. Это не имеет значения, поскольку, если показано, что дополнение проблемы неразрешимо, то сама проблема также не может быть разрешимой, так как рекурсивные языки замкнуты относительно дополнения. Алфавит цепочек данного экземпляра ПСП обозначим Σ, а алфавит индексных символов — I. В наших сведениях будем опираться на то, что LA, LB, LA и LB имеют КС-грамматики. КС-грамматики либо строятся непосредственно, либо вначале строятся МП-автоматы для языков-дополнений. 1. Пусть L(G1) = LA и L(G2) = LB. Тогда L(G1) I L(G2) — множество решений данного экземпляра ПСП. Пересечение пусто тогда и только тогда, когда решений нет. Отметим, что технически мы свели ПСП к языку, который состоит из пар КС-грамматик с непустым пересечением их языков, т.е. показали, что проблема “является ли пересечение языков двух КС-грамматик непустым?” неразрешима. Однако, как указано во введении к доказательству, доказать неразрешимость дополнения проблемы — это то же самое, что доказать неразрешимость самой проблемы. 2. Поскольку КС-языки замкнуты относительно объединения, то можно построить КС-грамматику G1 для  L'A U L'B . Поскольку (Σ U I) * — регулярное множество, то для него, конечно же, можно построить КС-грамматику G2. Далее,  L'A U L'B = LA I LB . Таким образом, в L(G1) отсутствуют лишь цепочки, которые представляют решения данного экземпляра ПСП. L(G2) содержит все цепочки из (Σ U I) * , так что эти языки совпадают тогда и только тогда, когда данный экземпляр ПСП не имеет решения. 3. Рассуждения точно такие же, как и в п. 2, но полагаем R регулярным выражением (Σ U I) * . 4. Достаточно рассуждений п. 3, поскольку Σ U I — единственный алфавит, замыканием которого может быть  L'A U L'B . 5. Пусть G1 — КС-грамматика для (Σ U I) * и G2 — КС-грамматика для  L'A U L'B . Следовательно, L(G1) ⊆ L(G2) тогда и только тогда, когда  L'A U L'B = (Σ U I) * , т.е. тогда и только тогда, когда данный экземпляр ПСП не имеет решения. 6. Рассуждения точно такие же, как и в п. 5, но полагаем R регулярным выражением (Σ U I) * , а L(G1) —  L'A U L'B"
  },
  {
  ...
  },
  {
    ...
    {
    "question": "Что такое накачка языка?",
    "answer": "Накачка языка - это возможность многократного повторения некоторой подстроки в любой строке подходящей длины любого бесконечного автоматного языка. Точная формулировка: Для каждой достаточно длинной строки в обычном языке должна существовать средняя часть, которую можно повторять (или наращивать) любое количество раз, чтобы получилась строка, которая всё ещё относится к этому языку. Используется для проверки принадлежности слова к определённому языку."
  },
  {
    "question": "Лемма о накачке КС языков",
    "answer": "Пусть G — КС-грамматика в форме Хомского. Тогда существует p ∈ N такое, что любое слово w ∈ L(G) длины не меньше p имеет представление вида x1y1zy2x2, где |y1y2| ⩾ 1, |y1zy2| ⩽ p, и все слова вида x1y^k1zy^k2x2 также принадлежат L(G)."
  },
  {
    "question": "Применение леммы о накачке КС языков с помощью двух противников",
    "answer": "Лемма о накачке для КС-языков, как и для регулярных языков, используется в виде “игры с противником” следующим образом. 1. Мы выбираем язык L, желая доказать, что он не контекстно-свободный. 2. Наш “противник” выбирает заранее неизвестное нам n, поэтому мы должны рассчитывать на любое возможное значение. 3. Мы выбираем z и при этом можем использовать n как параметр. 4. Противник разбивает z на uvwxy, соблюдая ограничения |vwx| ≤ n и vx ≠ ε. 5. Мы “выигрываем”, если можем, выбирая i и показывая, что uv^iwx^i y не принадлежит языку L."
  },
  {
    "question": "Лемма о накачке для регулярных языков",
    "answer": "Пусть L — регулярный язык. Существует константа n (зависящая от L), для которой каждую цепочку w из языка L, удовлетворяющую неравенству |w| ≥ n, можно разбить на три цепочки w = xyz так, что выполняются следующие условия. 1. y≠ε. 2. |xy| ≤ n. 3. Для любого k ≥ 0 цепочка x(y^k)z также принадлежит L. Это значит, что всегда можно найти такую цепочку y недалеко от начала цепочки w, которую можно “накачать”. Таким образом, если цепочку y повторить любое число раз или удалить (при k = 0), то результирующая цепочка все равно будет принадлежать языку L."
  ...
  {
    "question": "Применение леммы о накачке с помощью двух противников",
    "answer": "Применение леммы о накачке можно представить в виде игры со следующими правилами. 1 Игрок 1 выбирает язык L, нерегулярность которого нужно доказать. 2. Игрок 2 выбирает n, но не открывает его игроку 1; первый игрок должен построить игру для всех возможных значений n. 3. Игрок 1 выбирает цепочку w, которая может зависеть от n, причем ее длина должна быть не меньше n. 4. Игрок 2 разбивает цепочку w на x, y и z, соблюдая условия леммы о накачке, т.е. y ≠ ε и |xy| ≤ n. Опять-таки, он не обязан говорить первому игроку, чему равны x, y и z, хотя они должны удовлетворять условиям леммы. 5. Первый игрок “выигрывает”, если выбирает k, которое может быть функцией от n, x, y и z и для которого цепочка xy^k z не принадлежит L."
  },
  {
    "question": "Расскажи мне пожалуйста первую лемму о накачке",
    "answer": "Пусть G — КС-язык. Тогда существует p ∈ N такое, что любое слово w ∈ L(G) длины не меньше p имеет представление вида x1y1zy2x2, где |y1y2| ⩾ 1, |y1zy2| ⩽ p, и все слова вида x1y^k1zy^k2x2 также принадлежат L(G)."
  },
  {
    "question": "Расскажи мне пожалуйста вторую лемму о накачке",
    "answer": "Пусть G — КС-язык. Тогда существует p ∈ N такое, что любое слово w ∈ L(G) длины не меньше p имеет представление вида x1y1zy2x2, где |y2| ⩾ 1, |x1y1| ⩽ p, |y2| ⩽ p либо y2 накачивается отдельно, |x2| ⩽ p либо x2 накачивается отдельно, и все слова вида x1y^k1zy^k2x2 также принадлежат L(G)."
  },
  {
    "question": "Что такое нормальная форма Грейбах?",
    "answer": "Грамматика G (ε ∉ L(G)) находится в GNF (нормальной форме Грейбах), если каждое её правило имеет вид Ai → ajα, где Ai ∈ N,α ∈ N∗, aj ∈ Σ."
  },
  {
    "question": "Что такое лемма Огдена?",
    "answer": "Пусть L — КС-язык. Тогда существует такое число n, что в любом слове w, |w| ⩾ n, при отметке n или более букв, w представляется в виде x1y1zy2x2, причём либо во всех трех из x1, y1, z есть отмеченные буквы, либо они есть во всех трех из z, y2, x2, в слове y1zy2 отмечено не более n букв, и ∀k(x1y^k1zy^k2x2 ∈ L)."
  },
  {
    "question": "Может ли накачка применятся рекурсивно?",
    "answer": "Да, Понятие накачки может быть применено рекурсивно к некоторым достаточно длинным подсловам выбранного слова. То есть можно выкидывать из слова подслова, накачиваемые отдельно, без риска выйти из языка."
  },
  {
    "question": "Как реализуется алгоритм MemDFA для проверки принадлежности слова w к языку L в ДКА?",
    "answer": "Обозначив head(aw) = a и tail(aw) = w для a ∈ Σ и w ∈ Σ*, алгоритм выглядит следующим образом: MemDFA[A](w, q) Вход: ДКА A = (Q, Σ, δ, q0, F), состояние q ∈ Q, слово w ∈ Σ* Выход: Возвращает true, если w ∈ L(q), иначе false 1. Если w = ε, то возвращает q ∈ F 2. Иначе возвращает Member[A](δ(q, head(w)), tail(w)) Сложность алгоритма: O(|w|)"
  },
  {
    "question": "Как реализуется операция дополнения языка в ДКА?",
    "answer": "Реализация операции дополнения для ДКА проста. Поскольку ДКА имеет единственный прогон для каждого слова, который принимается, если автомат достигает заключительного состояния, можно просто поменять местами заключительные и незаключительные состояния. Таким образом, если слово не принималось в исходном автомате, оно будет принято в дополненном автомате, и наоборот. Этот алгоритм имеет линейную сложность: \\textbf{CompDFA(A)} \\\\ \\textbf{Вход:} ДКА $A = (Q, \\Sigma, \\delta, q_0, F)$ \\\\ \\textbf{Выход:} ДКА $B = (Q', \\Sigma, \\delta', q'_0, F')$ с $L(B) = \\overline{L(A)}$ \\\\ \\begin{algorithmic}[1] \\STATE $Q' \\leftarrow Q$; $\\delta' \\leftarrow \\delta$; $q'_0 \\leftarrow q_0$; $F' \\leftarrow \\emptyset$ \\FORALL{$q \\in Q$} \\IF{$q \\notin F$} \\STATE добавить $q$ в $F'$ \\ENDIF \\ENDFOR \\end{algorithmic}"
  },
  {
    "question": "Почему операция дополнения для ДКА сохраняет минимальность автомата?",
    "answer": "Операция дополнения для ДКА сохраняет минимальность, потому что каждое состояние дополненного автомата $Comp(A)$ распознаёт дополнение языка, которое распознавало то же состояние в исходном автомате $A$. Если состояния исходного автомата $A$ распознают попарно разные языки, то и состояния автомата $Comp(A)$ также будут распознавать разные языки. Согласно следствию, ДКА минимален, если его состояния распознают различные языки."
  },
  {
    "question": "Какова цель и структура универсального алгоритма для выполнения бинарных булевых операций над языками, распознаваемыми ДКА?",
    "answer": "Цель универсального алгоритма заключается в том, чтобы, принимая два ДКА, распознающих языки $L_1$ и $L_2$, и бинарную булевую операцию, возвращать ДКА, распознающий результат применения этой операции к языкам $L_1$ и $L_2$. Алгоритм обозначается как $BinOp[♢](A_1, A_2)$. \\textbf{BinOp[♢](A1, A2)} \\n \\textbf{Вход:} ДКА $A_1 = (Q_1, \\Sigma, \\delta_1, q_{01}, F_1)$, $A_2 = (Q_2, \\Sigma, \\delta_2, q_{02}, F_2)$ \\n \\textbf{Выход:} ДКА $A = (Q, \\Sigma, \\delta, Q_0, F)$ с $L(A) = L(A_1) \\; ♢ \\; L(A_2)$ \\n 1 $Q, \\delta, F \\leftarrow \\emptyset$ \\n 2 $q_0 \\leftarrow [q_{01}, q_{02}]$ \\n 3 $W \\leftarrow \\{q_0\\}$ \\n 4 \\textbf{while} $W \\neq \\emptyset$ \\textbf{do} \\n 5 \\quad pick $[q_1, q_2]$ from $W$ \\n 6 \\quad add $[q_1, q_2]$ to $Q$ \\n 7 \\quad \\textbf{if} $(q_1 \\in F_1) \\; ♢ \\; (q_2 \\in F_2)$ \\textbf{then} add $[q_1, q_2]$ to $F$ \\n 8 \\quad \\textbf{for all} $a \\in \\Sigma$ \\textbf{do} \\n 9 \\quad \\quad $q_{01} \\leftarrow \\delta_1(q_1, a)$; $q_{02} \\leftarrow \\delta_2(q_2, a)$ \\n 10 \\quad \\quad \\textbf{if} $[q_{01}, q_{02}] \\notin Q$ \\textbf{then} add $[q_{01}, q_{02}]$ to $W$ \\n 11 \\quad add $([q_1, q_2], a, [q_{01}, q_{02}])$ to $\\delta$"
  },
  {
    "question": "Каково количество состояний у результирующего ДКА при выполнении бинарной булевой операции над двумя ДКА, и в каких случаях минимальность не сохраняется?",
    "answer": "Выходной детерминированный конечный автомат (ДКА) от алгоритма $BinOp$ будет иметь $O(|Q_1| \\cdot |Q_2|)$ состояний, независимо от выполняемой булевой операции. Для демонстрации достижимости этого предела, можно рассмотреть алфавит $\\Sigma = \\{a\\}$ и язык $Mult_n$, который включает слова длиной, кратной $n$. Минимальный ДКА для языка $Mult_n$ представляет собой цикл из $n$ состояний, где начальное состояние также является единственным заключительным. Для любых двух взаимно простых чисел $n_1$ и $n_2$ выполняется равенство $Mult_{n_1} \\cap Mult_{n_2} = Mult(n_1 \\cdot n_2)$, что означает, что ДКА для $Mult(n_1 \\cdot n_2)$ будет иметь не менее $n_1 \\cdot n_2$ состояний. Однако в общем случае минимальность не сохраняется: произведение двух минимальных ДКА может не быть минимальным. Например, минимальный ДКА для регулярного языка $L \\cap L$ имеет одно состояние, в то время как результат построения произведения будет иметь то же количество состояний, что и минимальный ДКА для $L$."
  },
  {
    "question": "Как определяется, что ДКА принимает пустой язык, и какова сложность алгоритма проверки?",
    "answer": "ДКА принимает пустой язык тогда и только тогда, когда у него нет заключительных состояний. Алгоритм $Empty(A)$ возвращает $true$, если множество заключительных состояний $F$ пусто, и $false$ в противном случае. Если в реализации используется булевая переменная для хранения информации о наличии заключительных состояний, то сложность алгоритма составляет $O(1)$. Если требуется выполнить линейный поиск по множеству состояний, чтобы проверить $F = \\emptyset$, то сложность будет $O(|Q|)$, где $|Q|$ — это количество состояний автомата."
  },
  {
    "question": "Как определяется, что ДКА принимает универсальный язык Σ∗, и какова сложность алгоритма проверки?",
    "answer": "ДКА принимает универсальный язык $\\Sigma^*$ тогда и только тогда, когда все его состояния являются заключительными. Алгоритм $UnivDFA(A)$ возвращает $true$, если множество заключительных состояний $F$ совпадает с множеством всех состояний $Q$. Если автомат находится в нормальной форме, сложность алгоритма составляет $O(1)$. В противном случае сложность будет $O(|Q|)$, где $|Q|$ — это количество состояний автомата."
  },
  {
    "question": "Как алгоритм проверяет, что язык, распознаваемый одним ДКА, является подмножеством языка, распознаваемого другим ДКА?",
    "answer": "Алгоритм проверяет, что язык $L(A_1)$, распознаваемый ДКА $A_1$, является подмножеством языка $L(A_2)$, распознаваемого ДКА $A_2$, с помощью построения параллельного автомата $[A_1, A_2]$. Для каждого состояния $[q_1, q_2]$ автомата проверяется, что если состояние $q_1$ заключительное в $A_1$ ($q_1 \\in F_1$), то $q_2$ также должно быть заключительным в $A_2$ ($q_2 \\in F_2$). Если найдено состояние, где $q_1 \\in F_1$, а $q_2 \\notin F_2$, алгоритм возвращает $false$, что означает, что $L(A_1) \\not\\subseteq L(A_2)$. В противном случае, если таких состояний не найдено, алгоритм возвращает $true$, подтверждая, что $L(A_1) \\subseteq L(A_2)$. Сложность алгоритма зависит от количества состояний и алфавита."
  },
  {
    "question": "Как проверяется равенство языков двух ДКА?",
    "answer": "Равенство языков двух ДКА $L(A_1) = L(A_2)$ проверяется с помощью симметрической разности $L(A_1) \\Delta L(A_2)$. Если симметрическая разность пуста, то языки равны. Алгоритм $EqDFA$ модифицирует алгоритм $InclDFA$, проверяя, что для состояний $[q_1, q_2]$ выполняется одно из следующих условий: если $q_1 \\in F_1$ и $q_2 \\notin F_2$, или если $q_1 \\notin F_1$ и $q_2 \\in F_2$, то возвращается $false$, что означает, что языки не равны. В противном случае возвращается $true$."
  },
  {
    "question": "Какой альтернативный подход существует для проверки равенства двух ДКА, помимо алгоритма EqDFA?",
    "answer": "Альтернативный подход заключается в минимизации двух ДКА $A_1$ и $A_2$, а затем проверке, являются ли они изоморфными. Проверка изоморфизма может быть пропущена, если применить алгоритм $CSR$ к объединению двух НКА $A_1 \\cup A_2$ и разделить состояния на блоки. Языки равны, если начальные состояния обоих автоматов находятся в одном блоке после завершения алгоритма."
  },
  {
    "question": "Какая сложность проверки равенства языков двух ДКА с использованием алгоритма Хопкрофта для вычисления CSR?",
    "answer": "Если используется алгоритм Хопкрофта для вычисления CSR, то проверка равенства языков может быть выполнена за время $O(n \\log n)$, где $n$ — это сумма состояний автоматов $A_1$ и $A_2$."
  },
  {
    "question": "Какие два важных преимущества имеет алгоритм EqDFA по сравнению с минимизацией автоматов?",
    "answer": "Первое преимущество заключается в том, что алгоритм $EqDFA$ работает \"на лету\" (on-the-fly), позволяя тестировать равенство языков, пока строятся автоматы $A_1$ и $A_2$, и останавливать работу при нахождении различий. Второе преимущество — это возможность модифицировать $EqDFA$ так, чтобы он возвращал свидетельство (witness), то есть слово, которое находится в симметрической разности $L(A_1)$ и $L(A_2)$, если языки не равны."
  }
]
`
