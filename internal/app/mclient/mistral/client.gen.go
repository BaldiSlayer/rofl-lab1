...
// GetChatResponseResponse defines model for GetChatResponseResponse.
type GetChatResponseResponse struct {
...
type ApiGetChatResponseGetChatResponsePostResponse struct {
	Body         []byte
	HTTPResponse *http.Response
	JSON200      *GetChatResponseResponse
	JSON422      *HTTPValidationError
}
...
func ParseApiGetChatResponseGetChatResponsePostResponse(rsp *http.Response) (*ApiGetChatResponseGetChatResponsePostResponse, error) {
	...
	switch {
	case strings.Contains(rsp.Header.Get("Content-Type"), "json") && rsp.StatusCode == 200:
		var dest GetChatResponseResponse
		if err := json.Unmarshal(bodyBytes, &dest); err != nil {
			return nil, err
		}
		response.JSON200 = &dest
