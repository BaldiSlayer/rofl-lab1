...
**Вопрос:** Какие теоремы есть про проблему выводимости слов в теории формальных языков?


**Ответ:** Существует алгоритм, позволяющий по произвольной контекстно-свободной грамматике G узнать, верно ли, что ε ∈ L(G). Существует алгоритм, позволяющий по произвольной контекстно-свободной грамматике G и по сло-ву w узнать, верно ли, что w ∈ L(G).


**Вопрос:** Когда проблема пустоты языка разрешима в теории формальных языков?


**Ответ:** Существует алгоритм, позволяющий по произвольной контекстно-свободной грамматике G узнать, верно ли, что L(G) = ∅.


**Вопрос:** О чём проблема бесконечности языка в теории формальных языков?


**Ответ:** Она связана с вопросом о том, можно ли за полиномиальное время определить, является ли язык формального символа бесконечным или конечным.


**Вопрос:** Когда разрешима проблема равентства автоматных языков в теории формальных языков?


**Ответ:** Существует алгоритм, позволяющий по произвольным двум праволинейным грамматикам G1 и G2 узнать, верно ли, что L(G1) ⊆ L(G2). Существует алгоритм, позволяющий по произвольным двум праволинейным грамматикам G1 и G2 узнать, верно ли, что L(G1) = L(G2).


**Вопрос:** Каждая ли контекстная грамматика является неукорачивающей и наоборот в теории формальных языков?


**Ответ:** Каждая контекстная грамматика является неукорачивающей. Каждая неукорачивающая грамматика эквивалентна некоторой контекстной грамматике.


**Вопрос:** Какой критерий при котором язык L, не содержащий пустого слова, порождается некоторой неукорачивающей грамматикой в теории формальных языков?


**Ответ:** Язык L, не содержащий пустого слова, порождается некоторой неукорачивающей грамматикой тогда и только тогда, когда существует линейно ограниченный автомат (в общем случае недетерминированный), который допускает язык L.


**Вопрос:** Является ли класс языков, порождаемых неукорачивающими грамматиками, замкнутым относительно каких-то операций в теории формальных языков?


**Ответ:** Класс языков, порождаемых неукорачивающими грамматиками, замкнут относительно операций объединения, пересечения и дополнения.
