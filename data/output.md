...
**Ответ:** Порождения из стартового символа грамматики приводят к цепочкам, которые называются 'выводимыми цепочками' ('sentential form'). Пусть G = (V, T, P, S) — контекстно-свободная грамматика, где V — множество нетерминалов, T — терминалов, P — множество продукций, S — стартовый символ. Тогда любая цепочка α из (V U T)*, для которой S ⇒ α (*), называется выводимой цепочкой. Обозначение * означает, что переходы могут быть многократными. Если S ⇒ (lm)α (*) , то α является левовыводимой цепочкой, а если S ⇒ (rm)α (*), то — правовыводимой. Язык L(G) образуют выводимые цепочки из T*, состоящие исключительно из терминалов. T* - это множество всех возможных цепочек из терминалов Т (замыкание Клини).


**Вопрос:** Какие основные свойства дерева разбора для контекстно-свободной грамматики?


**Ответ:** G = (V, T, P, S) — контекстно-свободная грамматика, где V — множество нетерминалов, T — терминалов, P — множество продукций, S — стартовый символ. Деревья разбора для G — это деревья со следующими свойствами: каждый внутренний узел отмечен нетерминалом из V; каждый лист отмечен либо нетерминалом, либо терминалом, либо ε. При этом, если лист отмечен ε, он должен быть единственным сыном своего родителя; если внутренний узел отмечен A, и его сыновья отмечены слева направо X_1, X_2, …, X_k, соответственно, то A → X_1,X_2 ⋅⋅⋅ X_k является продукцией в P. X может быть ε лишь в одном случае — если он отмечает единственного сына, и A → ε — продукция грамматики G.


**Вопрос:** Может ли цепочка 𝜔, порожденная контекстно-свободной грамматикой G = (V, T, P, S), иметь два разных дерева разбора, если у нее только одно левое порождение из P? T — множество терминальных символов, V — множество нетерминальных символов, S — стартовый символ, P — множество правил продукций.


**Ответ:** Цепочка 𝜔 имеет два разных дерева разбора тогда и только тогда, когда 𝜔 имеет два разных левых порождения из P.


**Вопрос:** Пусть G = (V, T, P, S) — контекстно-свободная грамматика, где V — множество нетерминалов, T — терминалов, P — множество продукций, S — стартовый символ. Если рекурсивный вывод утверждает, что терминальная цепочка w принадлежит языку нетерминала 𝐴, что можно сказать о дереве разбора, связанной с этим нетерминалом?


**Ответ:** Пусть G = (V, T, P, S) — контекстно-свободная грамматика, где V — множество нетерминалов, T — терминалов, P — множество продукций, S — стартовый символ. Если рекурсивный вывод утверждает, что терминальная цепочка w принадлежит языку нетерминала A, то существует дерево разбора с корнем A и кроной w.
...
**Вопрос:** Если контекстно-свободная грамматика 𝐺1 строится из грамматики G путем удаления цепных продукций, верно ли что 𝐿(𝐺1) = 𝐿(𝐺), где L - контекстно-свободный язык, порожденный грамматикой G?


**Ответ:** Если контекстно-свободная грамматика 𝐺1 строится из грамматики G путем удаления цепных продукций, то 𝐿(𝐺1) = 𝐿(𝐺), где L - контекстно-свободный язык, порожденный грамматикой G.
...
**Вопрос:** Если L - контекстно-свободный язык и h - гомоморфизм, то является ли обратный гомоморфизм h(L)^-1 также контекстно-свободным языком?


**Ответ:** Если L - контекстно-свободный язык и h - гомоморфизм, то обратный гомоморфизм h(L)^-1 также является контекстно-свободным языком.


**Вопрос:** Относительно каких операций замкнуты контекстно-свободные языки?


**Ответ:** Контекстно-свободные языки замкнуты относительно подстановки, объединения, конкатенации, обращения и обратного гомоморфизма. Контекстно-свободные языки не замкнуты относительно пересечения и дополнения, но пересечение контекстно-свободного языка с регулярным всегда является контекстно-свободным языком.


**Вопрос:** Как эффективно проверить принадлежность цепочки 𝜔 контекстно-свободному языку L?
