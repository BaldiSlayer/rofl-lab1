...
**Вопрос:** Какие операторы регулярных выражений дистрибутивны? А ассоциативны?


**Ответ:** Объединение и конкатенация ассоциативны. Коммутативно только объединение.


**Вопрос:** Закон идемпотентности операции объединения
...
**Ответ:** Обращением строки (a_1)(a_2)...(a_n) называется строка, записанная в обратном порядке, т.е. (a_n)(a_n-1)...(a_1). Обращение w обозначается w^R. Таким образом, например, (0010)^R есть 0100, а ε^R = ε.


**Вопрос:** Что такое обращение языка L в регулярных языках?


**Ответ:** Обращение языка L, обозначаемое L^R, состоит из всех строк, обратных строкам языка L. Например, если L = {001, 10, 111}, то L^R = {100, 01, 111}.


**Вопрос:** Что такое Гомоморфизм строк в регулярных языках?


**Ответ:** Гомоморфизм строк — это такая функция на множестве строк, которая подставляет определенную строку вместо каждого символа данной строки.
